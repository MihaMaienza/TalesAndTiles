...
<%@ page import="model.DAO.GenereGiocoDAO, model.DAO.GenereLibroDAO" %>
...
<%
    ...
    String tipo = request.getParameter("tipo"); // tipo può essere "gioco" o "libro"
	...
	String nomeGenere="";
	String tipoPlurale="";
    // Ottieni i prodotti in base al genere e al tipo
    if ("gioco".equalsIgnoreCase(tipo)) {
        prodotti = prodottoDAO.getAllProductsByIdGenereGioco(idGenere);
        nomeGenere = GenereGiocoDAO.getNomeGenereGioco(idGenere);
        tipoPlurale = "giochi ";
    } else if ("libro".equalsIgnoreCase(tipo)) {
        prodotti = prodottoDAO.getAllProductsByIdGenereLibro(idGenere);
        nomeGenere = GenereLibroDAO.getNomeGenereLibro(idGenere);
        tipoPlurale = "libri ";
    }
...
<div class="titolo">
    <h1>Catalogo <%= tipoPlurale %><%=nomeGenere %></h1>
    <!-- Dropdown per l'ordinamento -->
...
<div class="containerP" id="product-container">
    ...
    <div class="card" data-price="<%= p.getPrezzo() %>">
        ...
        <div class="contentBx">
            ...
            <button class="add-to-cart-btn" data-product-id="<%= p.getId() %>">Aggiungi al carrello</button>
        </div>
    </div>
    <%
        }
